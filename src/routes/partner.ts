...
const normalizeAccountId = (raw?: string) => {
  const t = (raw ?? "").trim();
  if (isEvm(t)) return { evm: t.toLowerCase(), hedera: null as string | null };
...
router.post("/auth/signin/nonce", async (req, res) => {
  const { kind, accountId, network, chainId } = req.body ?? {};
  if (!kind || !accountId) return res.status(400).json({ error: "Missing wallet" });

  // Optional: early format guard
  const { evm, hedera } = normalizeAccountId(accountId);
  if (!evm && !hedera) return res.status(400).json({ error: "Invalid wallet format" });
...
router.post("/auth/signin/verify", async (req, res) => {
  const { kind, accountId, network, chainId, signature, nonce } = req.body ?? {};
  ...
  if (rec.type !== kind) return res.status(401).json({ error: "Kind mismatch" });
...
router.post("/keys/:id/regenerate",
    ...
    async (req, res) => {
        ...
        const newKey = await issueApiKey(prisma, kms, {
            ...
            scopes: cur.scopes.map(s => s.scope),
